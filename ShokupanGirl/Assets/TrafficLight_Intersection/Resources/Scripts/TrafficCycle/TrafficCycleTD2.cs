...
/*	
 *	時差式（矢印2方向）の信号サイクルスクリプト
 */

public class TrafficCycleTD2 : TrafficCycle {
	...
	void Start () {
		...
		tlas1 = new TrafficLightArrowSetting[direction1E.Length, 2];
		for (int i = 0; i < tlas1.GetLength(0); i++) {
			tlas1[i, 0] = direction1E [i].transform.Find ("TrafficLightArrow_R").GetComponent<TrafficLightArrowSetting> ();
			tlas1[i, 1] = direction1E [i].transform.Find ("TrafficLightArrow_C").GetComponent<TrafficLightArrowSetting> ();
		}

		//各灯火の設定
		light1 = new GameObject[direction1.Length, 3];
		light1E = new GameObject[direction1E.Length, 3];
		lightA1 = new GameObject[direction1E.Length, 2]; 
        ...
        //矢印灯(lightA1) 0:右折 1:直進
        for (int i = 0; i < light1E.GetLength (0); i++) {
			if (tlas1 [i, 0].lightType == 1) {
				lightA1 [i, 0] = direction1E [i].transform.Find ("TrafficLightArrow_R/TLightArrow_R").gameObject;
			} else if (tlas1 [i, 0].lightType == 0 || tlas1 [i, 0].lightType == 2){
				lightA1 [i, 0] = direction1E [i].transform.Find ("TrafficLightArrow_R/TLightGroupArrow_R/PointLightA").gameObject;
			}
			if (tlas1 [i, 1].lightType == 1) {
				lightA1 [i, 1] = direction1E [i].transform.Find ("TrafficLightArrow_C/TLightArrow_C").gameObject;
			} else if (tlas1 [i, 1].lightType == 0 || tlas1 [i, 1].lightType == 2) {
				lightA1 [i, 1] = direction1E [i].transform.Find ("TrafficLightArrow_C/TLightGroupArrow_C/PointLightA").gameObject;
			}
		...
		//赤時間(2) ＝ 青時間(1) + 延長時間(1) + 黄時間(1)(2) + 全赤時間
		redTime2 = greenTime1 + extraTime1 + yellowTime + yellowTime2 + allRedTime;
	...
	void FixedUpdate(){
		...
		for(int i = 0; i < light1E.GetLength(0); i++){
			if (cTime < greenTime1) {
				...
				tlas1[i, 1].SettingLightA (lightA1[i, 1], false);
			} else if(cTime < (greenTime1 + yellowTime)){
				//延長側を黄+上矢印に
				...
				tls1E [i].SettingLightR (light1E [i, 2], false);
				tlas1 [i, 0].SettingLightA (lightA1 [i, 0], false);
				tlas1 [i, 1].SettingLightA (lightA1 [i, 1], true);
