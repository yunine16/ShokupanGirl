...
/*	
 *	時差式（矢印3方向・黄と赤で分割表示）の信号サイクルスクリプト
 */

public class TrafficCycleTD3 : TrafficCycle {
    ...
    //灯火のゲームオブジェクト
    private GameObject[,] light1E, lightA1;


	//車灯用の時間
	//方向1青時間、方向2青時間、黄時間(1)(2)、全赤時間、方向1延長時間
	public float greenTime1, greenTime2, yellowTime, yellowTime2, allRedTime, extraTime1;
	//歩灯用の時間
	...
	void Start () {
		...
		tlas1 = new TrafficLightArrowSetting[direction1E.Length, 1];
		for (int i = 0; i < tlas1.GetLength(0); i++) {
			tlas1[i, 0] = direction1E [i].transform.Find ("TrafficLightArrows3").GetComponent<TrafficLightArrowSetting> ();
		}

		//各灯火の設定
		light1 = new GameObject[direction1.Length, 3];
		light1E = new GameObject[direction1E.Length, 3];
		lightA1 = new GameObject[direction1E.Length, 3]; 
        ...
        //矢印灯(lightA1) 0:左折 1:直進 2:右折
        for (int i = 0; i < light1E.GetLength (0); i++) {
			if (tlas1 [i, 0].lightType == 1) {
				lightA1 [i, 0] = direction1E [i].transform.Find ("TrafficLightArrows3/TLightArrow_L").gameObject;
				lightA1 [i, 1] = direction1E [i].transform.Find ("TrafficLightArrows3/TLightArrow_C").gameObject;
				lightA1 [i, 2] = direction1E [i].transform.Find ("TrafficLightArrows3/TLightArrow_R").gameObject;
			} else if (tlas1 [i, 0].lightType == 0 || tlas1 [i, 0].lightType == 2){
				lightA1 [i, 0] = direction1E [i].transform.Find ("TrafficLightArrows3/TLightGroupArrow_L/PointLightA").gameObject;
				lightA1 [i, 1] = direction1E [i].transform.Find ("TrafficLightArrows3/TLightGroupArrow_C/PointLightA").gameObject;
				lightA1 [i, 2] = direction1E [i].transform.Find ("TrafficLightArrows3/TLightGroupArrow_R/PointLightA").gameObject;
			}
		}
		...
		//歩灯赤時間(1)
		if (directionP1.Length != 0) {
			pRedTime1 = redTime1 + extraTime1 + yellowTime + yellowTime2 + (greenTime1 - minimumTime1 + CONSTS.CHANGE_TIME);
		}
		//歩灯赤時間(2)
		if (directionP2.Length != 0) {
		...
		//1サイクルの時間
		allTime = greenTime1 + extraTime1 + yellowTime + yellowTime2 + redTime1;
	...
	void FixedUpdate(){
		...
		for(int i = 0; i < light1E.GetLength(0); i++){
			if (cTime < greenTime1) {
				...
				tlas1 [i, 0].SettingLightA (lightA1 [i, 2], false);
			} else if (cTime < (greenTime1 + extraTime1 + yellowTime)) {
				//延長側を赤+全矢印に
				tls1E[i].SettingLightG (light1E [i, 0], false);
				tls1E[i].SettingLightY (light1E [i, 1], false);
			...
			} else if (cTime < (greenTime1 + extraTime1 + yellowTime + yellowTime2)){
				//延長側を黄信号に
				tls1E[i].SettingLightG (light1E [i, 0], false);
				tls1E[i].SettingLightY (light1E [i, 1], true);
				tls1E[i].SettingLightR (light1E [i, 2], false);
