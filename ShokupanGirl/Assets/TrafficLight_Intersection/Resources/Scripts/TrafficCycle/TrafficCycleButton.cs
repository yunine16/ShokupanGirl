...
public class TrafficCycleButton : TrafficCycle {
    ...
    private float updateGreenTime1 = 0f;    //車灯が実際に変わる時間
	...
	void Start () {
		...
		//赤時間(1)
		redTime1 = allRedTime * 2 + pGreenTime2 + CONSTS.BLINK_TIME * 2 * pBlinkTimes2;
		//歩灯赤時間(2) ※ボタンを押してからの赤時間
		pRedTime2 = changeTime + yellowTime + allRedTime;
		//1サイクルの時間
		afterPushTime = pRedTime2 + redTime1 - allRedTime;
	...
	void FixedUpdate(){
		...
		//方向2歩灯の信号サイクル
		if (cStep2 == 0) {
			if (cTime2 >= pRedTime2 && cTime2 < pRedTime2 + pGreenTime2) {
				for (int i = 0; i < lightP2.GetLength (0); i++) {
					//青信号に
					tlps2[i].SettingLightPR (lightP2 [i, 0], false);
					tlps2[i].SettingLightPG (lightP2 [i, 1], true);
				} 
			} else if (cTime2 >= pRedTime2 + pGreenTime2){
				cStep2 = 1;		//点滅ステップへ
				if (audioOn) {
					for (int j = 0; j < audioDevice2.Length; j++) {
			...
			if (cTime2 > pRedTime2 + pGreenTime2 || cTime2 < pRedTime2) {
				for (int i = 0; i < lightP2.GetLength (0); i++) {
					//赤信号に
			...
			if (!pushed) {	//ボタンが押されていなければ
				...
				if (Input.GetKey (KeyCode.Space)) {
					...
					//1：車両用の最低限の青時間を待ってから変化
						...
						pRedTime2 = updateGreenTime1 + yellowTime + allRedTime;
						//1サイクルの時間の更新
						afterPushTime = pRedTime2 + redTime1 - allRedTime;
					} 
					//2：一定の信号サイクルに合わせて変化
